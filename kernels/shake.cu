...
extern "C" __global__ void shake_positions(
    ...
    unsigned int n_groups)
{
  // Group-contiguous shared-memory staging (mirrors rattle_velocities;
  // see `rqm/integration/shake.md` *RATTLE Shared-Memory Staging*). One
  // thread per group; the block owns a contiguous slice of `group_atoms`
  // and stages its atoms into dynamic shared memory with coalesced
  // global reads. The SHAKE solve and the virial run on the
  // shared-resident per-atom state, the final lab-frame positions and
  // velocities are written into shared, and the block stores them back
  // coalesced. The per-group floating-point operation sequence is
  // unchanged, so results are bit-identical to a per-thread gather.
  // rq-115e5926
  ...
  unsigned int G0 = blockIdx.x * blockDim.x;
  ...
  // 16 per-atom shared arrays. s_px/s_py/s_pz hold the raw post-drift
  // positions on load and the final lab-frame positions on store; s_vx/
  // s_vy/s_vz likewise for velocities. s_w carries the charge through.
  Real *s_px  = smem;
  Real *s_py  = s_px  + n_block_atoms;
  Real *s_pz  = s_py  + n_block_atoms;
  Real *s_w   = s_pz  + n_block_atoms;
  Real *s_x0  = s_w   + n_block_atoms;
  Real *s_y0  = s_x0  + n_block_atoms;
  Real *s_z0  = s_y0  + n_block_atoms;
  Real *s_xc  = s_z0  + n_block_atoms;
  Real *s_yc  = s_xc  + n_block_atoms;
  Real *s_zc  = s_yc  + n_block_atoms;
  Real *s_vx  = s_zc  + n_block_atoms;
  Real *s_vy  = s_vx  + n_block_atoms;
  Real *s_vz  = s_vy  + n_block_atoms;
  Real *s_m   = s_vz  + n_block_atoms;
  Real *s_im  = s_m   + n_block_atoms;
  Real *s_vir = s_im  + n_block_atoms;

  // Coalesced staging load. snapshot_* are indexed by atom slot, which
  // equals atom_base + t for the block's contiguous range.
  for (unsigned int t = threadIdx.x; t < n_block_atoms; t += blockDim.x) {
    unsigned int i = group_atoms[atom_base + t];
    Real4 pq = posq[i];
    s_px[t] = pq.x; s_py[t] = pq.y; s_pz[t] = pq.z; s_w[t] = pq.w;
    s_x0[t] = snapshot_x[atom_base + t];
    s_y0[t] = snapshot_y[atom_base + t];
    s_z0[t] = snapshot_z[atom_base + t];
  ...
  if (active) {
    ...
    unsigned int lb = aoff - atom_base;

    // Per-group shared views (disjoint atom ranges → no synchronisation
    // needed within the solve).
    Real *x0 = &s_x0[lb], *y0 = &s_y0[lb], *z0 = &s_z0[lb];
    Real *x_c = &s_xc[lb], *y_c = &s_yc[lb], *z_c = &s_zc[lb];
    Real *m_atom = &s_m[lb], *inv_m = &s_im[lb];

    // Raw post-drift positions (kept for the delta-style write-back) and
    // their image-aligned copies. Small per-thread arrays.
    ...
    // Update half-step velocities and compute the final lab-frame
    // positions into shared. The image-shift fix-up operated on local
    // copies; we re-apply the same delta to the original global
    // positions so the global image bookkeeping stays valid:
    // x_global_new = x_global_old + (x_c_local - x_u_local).
    ...
    for (unsigned int a = 0; a < acnt; ++a) {
      ...
      s_vx[lb + a] += dxg * inv_dt;
      s_vy[lb + a] += dyg * inv_dt;
      s_vz[lb + a] += dzg * inv_dt;
      if (a == 0) {
        // Atom 0 is the reference image; its lab-frame position equals
        // the local-frame x_c[0].
        s_px[lb + 0] = x_c[0];
        s_py[lb + 0] = y_c[0];
        s_pz[lb + 0] = z_c[0];
      } else {
        // Atoms 1..n-1 use the delta-style update so the global image
        // bookkeeping stays valid (s_p* still hold the raw positions).
        s_px[lb + a] += dxg;
        s_py[lb + a] += dyg;
        s_pz[lb + a] += dzg;
      }
      ...
      s_vir[lb + a] = scale * (dxg * rx + dyg * ry + dzg * rz);
    }
  }
  __syncthreads();
  ...
  for (unsigned int t = threadIdx.x; t < n_block_atoms; t += blockDim.x) {
    ...
    constraint_virial[atom_base + t] = s_vir[t];
  }
}
